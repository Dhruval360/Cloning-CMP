...
/* All outputs (including the help page and the version have been copied directly from the output screen) */
...
terminal input). Hence, by default argv[0] will be the name of the executable form of this program i.e ./a.out or any other name we give
to it.
*/
	{
	...
	if(check == no_errors)
		check = output(argv, filename_indices, ignore_initial_value, options_chosen, limit, &same, filenames); 	
